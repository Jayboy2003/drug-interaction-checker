...
3. Connect the GitHub repo, pick branch `main`, and set
   **Main file path:** `streamlit_app.py`.
4. Expand **Advanced settings** → **Secrets** and paste:
   ```
   OPENAI_API_KEY = "sk-your-key-here"
   ```
   (Streamlit stores this as `st.secrets`/env — it is never in the repo.)
5. Deploy → use the generated `https://...streamlit.app` link as your live link.
...
├── frontend/index.html        # web UI (FastAPI mode)
├── streamlit_app.py            # Streamlit UI (recommended deploy)
├── requirements.txt
